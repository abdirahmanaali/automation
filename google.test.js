...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("basketball" ,Key.RETURN);
  await driver.wait(until.titleIs("basketball - Google Search"), 1000)
  ...
  await driver.findElement(By.name("q")).sendKeys("soccer", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("soccer - Google Search", 1000));
});
